...
def make_validation_table(fitspath, bin_pts_fname):
    massbin_table = asc.read(fitspath + bin_pts_fname + '_massbin.tbl', format = 'fixed_width_two_line')
    em_table = asc.read(fitspath + bin_pts_fname + '_massbin_emission_lines.tbl')
    ...
    detections = np.zeros(len(ID))            #one means detection, zero means non-detection
    
    ##old method
    '''
    if bin_pts_fname == '800':
        detections[1] = 0
        detections[4] = 0
        
    if bin_pts_fname == '75_112_113_300_600_1444_1444':
        detections[1] = 0
        detections[5] = 0
        detections[6] = 0   
        
    if bin_pts_fname == 'revised_75_112_113_300_600_1444_1444':
        detections[3] = 0
        detections[5] = 0
        detections[6] = 0
        
    if bin_pts_fname == 'hbeta_revised_75_112_113_300_600_1444_1444':
        detections[2] = 0
        detections[8] = 0
        detections[10] = 0
        detections[12] = 0  
    ...
    out_ascii = fitspath + bin_pts_fname + '_updated_emission_lines.tbl'
    updated_em_table = Table(em_table)
    updated_em_table['OIII_4363_Flux_Observed'] = updated_O_4363_flux
    detection_col = Column(detections, name = 'Detection')
    updated_em_table.add_column(detection_col)
    ...
    out_ascii = fitspath + bin_pts_fname + '_massbin_validation.tbl'
