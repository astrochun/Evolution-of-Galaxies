...
def make_validation_table(fitspath, log=None):
    """
    Purpose:
    ...
    bin_table = asc.read(bin_info_file, format='fixed_width_two_line')

    bin_fit_file = fitspath + filename_dict['bin_fit']
    log.info(f"Reading: {bin_fit_file}")
    em_table = asc.read(bin_fit_file)
    ...
    # 1.0 --> detection, 0.0 --> non-detection,
    # 0.5 --> non-detection with reliable 5007 or detection with wide line
    detections = np.zeros(len(ID))
    valid_stacks_idx = np.where((O_4363_SN >= 3) &
                                (O_5007_SN > 100) & (O_4363_sigma < 2))[0]
    reliable_5007_stacks = np.where((O_4363_SN < 3) & (O_5007_SN > 100))[0]
    wide_lines_valid = np.where((O_4363_SN >= 3) &
                                (O_5007_SN > 100) & (O_4363_sigma >= 2))[0]
    ...
    valid_table_file = fitspath + filename_dict['bin_valid']
    log.info(f"Writing: {valid_table_file}")
    asc.write(valid_table, valid_table_file, format='fixed_width_two_line',
              overwrite=True)


def quality_assurance(bin_type_str, QA_flag, log=None):
    """
    Purpose:
