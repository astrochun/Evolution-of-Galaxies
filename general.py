...
from .Analysis import library, emission_line_fit, indiv_gals
from .Plotting.composite_plots import bin_derived_props_plots
from .Plotting.individual_plots import indiv_derived_props_plots,\
    indiv_metal_mass_plot, get_indiv_detect
...
def run_bin_analysis(valid_rev=False, apply_dust=False, err_prop=False,
    ...
    if bin_type.lower() == 'mass':
        ...
        log.warning(f"Invalid binning type! {bin_type}")
        sys.exit(0)

    HB_lum = get_HB_luminosity(log=log)
    ...
    result_revised = asc.read(deep2_revised_file)
    ...
    log.info(f"Writing: {flux_fits_file}")
    fits.writeto(flux_fits_file, flux, hdr)

    # Run emission line fits
    log.info(f"Reading: {flux_fits_file}")
    ...
    emission_line_fit.zm_general(fitspath, Spect_1D, dispersion, wave,
                                 lambda0, line_type, line_name,
                                 s, a, c, s1, a1, s2, a2,
                                 hbeta_bin=bool_hbeta_bin, log=log)

    # Run validation table
    valid_table.make_validation_table(fitspath)  # log=log)
    ...
    log.info(f"Reading: {valid_rev_file}")
    asc.write(vtbl, valid_rev_file, format='fixed_width_two_line',
              overwrite=True)
...
def run_indiv_metal_mass_plot(fitspathM, fitspathMLHb, restrictMTO=False,
                              revised_files=False, error_bars=False, log=None):
    """
    Purpose:
    ...
    # M-only stacks
    indv_derived_prop_file = path_init + fitspathM + \
                             filename_dict['indv_derived_prop']
    log.info(f"Reading: {indv_derived_prop_file}")
    Mbin_indiv_derivedprops_tbl = asc.read(indv_derived_prop_file,
                                           format='fixed_width_two_line')

    indv_prop_file = path_init + fitspathM + filename_dict['indv_prop']
    log.info(f"Reading: {indv_prop_file}")
    Mbin_indiv_props_tbl = asc.read(indv_prop_file,
                                    format='fixed_width_two_line')

    indv_bin_info_file = path_init + fitspathM + filename_dict['indv_bin_info']
    Mbin_indiv_bininfo_tbl = asc.read(indv_bin_info_file,
                                      format='fixed_width_two_line')

    # M-LHb stacks
    indv_derived_prop_file2 = path_init + fitspathMLHb + \
                              filename_dict['indv_derived_prop']
    log.info(f"Reading: {indv_derived_prop_file2}")
    MLHbbin_indiv_derivedprops_tbl = asc.read(indv_derived_prop_file2,
                                              format='fixed_width_two_line')

    indv_prop_file2 = path_init + fitspathMLHb + filename_dict['indv_prop']
    log.info(f"Reading: {indv_prop_file2}")
    MLHbbin_indiv_props_tbl = asc.read(indv_prop_file2,
                                       format='fixed_width_two_line')

    indv_bin_info_file2 = path_init + fitspathMLHb + \
                          filename_dict['indv_bin_info']
    log.info(f"Reading: {indv_prop_file2}")
    MLHbbin_indiv_bininfo_tbl = asc.read(indv_bin_info_file2,
                                         format='fixed_width_two_line')

    # Read in composite data tables
    bin_valid_file = path_init + fitspathM + filename_dict['bin_valid']
    log.info(f"Reading: {bin_valid_file}")
    Mbin_valid_tbl = asc.read(bin_valid_file,
                              format='fixed_width_two_line')

    bin_info_file = path_init + fitspathM + filename_dict['bin_info']
    log.info(f"Reading: {bin_info_file}")
    Mbininfo_tbl = asc.read(bin_info_file, format='fixed_width_two_line')

    # M-LHb stacks
    bin_valid_file2 = path_init + fitspathMLHb + filename_dict['bin_valid']
    log.info(f"Reading: {bin_valid_file2}")
    MLHbbin_valid_tbl = asc.read(bin_valid_file2,
                                 format='fixed_width_two_line')

    bin_info_file2 = path_init + fitspathMLHb + filename_dict['bin_info']
    log.info(f"Reading: {bin_info_file2}")
    MLHbbininfo_tbl = asc.read(bin_info_file2,
                               format='fixed_width_two_line')

    if revised_files:
        bin_derived_prop_file = path_init + fitspathM + \
                                filename_dict['bin_derived_prop_rev']
        bin_derived_prop_file2 = path_init + fitspathMLHb + \
                                 filename_dict['bin_derived_prop_rev']
    else:
        bin_derived_prop_file = path_init + fitspathM + \
    ...
    Mbin_derivedprops_tbl = asc.read(bin_derived_prop_file,
                                     format='fixed_width_two_line')
    log.info(f"Reading: {bin_derived_prop_file2}")
    MLHbbin_derivedprops_tbl = asc.read(bin_derived_prop_file2,
                                        format='fixed_width_two_line')
    ...
    Mbin_indiv_detect, \
        Mbin_indiv_nondetect = get_indiv_detect(Mbin_indiv_props_tbl,
                                                Mbin_indiv_bin_detect, log=None)
    MLHbbin_indiv_detect, \
        MLHbbin_indiv_nondetect = get_indiv_detect(MLHbbin_indiv_props_tbl,
                                                   MLHbbin_indiv_bin_detect,
                                                   LHbeta_bins=True, log=None)

    # Define mass bin and mass-LHbeta bin dictionaries
    Mbin_dict = {
        'path': path_init + fitspathM,
        'composite_logM': Mbin_logM,
        'composite_detect': Mbin_detect,
        'composite_nondetect': Mbin_nondetect,
        'composite_metallicity': Mbin_metal,
        'indiv_logM': Mbin_indiv_logM,
        'indiv_detect': Mbin_indiv_detect,
        'indiv_nondetect': Mbin_indiv_nondetect,
        'indiv_metallicity': Mbin_indiv_metal
    }
    MLHbbin_dict = {
        'path': path_init + fitspathMLHb,
        'composite_logM': MLHbbin_logM,
        'composite_detect': MLHbbin_detect,
        'composite_nondetect': MLHbbin_nondetect,
        'composite_metallicity': MLHbbin_metal,
        'indiv_logM': MLHbbin_indiv_logM,
        'indiv_detect': MLHbbin_indiv_detect,
        'indiv_nondetect': MLHbbin_indiv_nondetect,
    ...
    indiv_metal_mass_plot(Mbin_dict, MLHbbin_dict, restrictMTO=restrictMTO,
                          revised=revised_files, err_bars=error_bars, log=None)
