...
def bin_derived_props_plots(fitspath, hbeta_bin=False, err_bars=False, revised=False):
    ...
    if revised:
        ...
        valid_table = asc.read(fitspath + filename_dict['bin_valid_rev'], format='fixed_width_two_line')
    else:
        pdf_pages = PdfPages(fitspath + filename_dict['bin_derived_prop'].replace('.tbl', '.pdf'))
        metal_table = asc.read(fitspath + filename_dict['bin_derived_prop'], format='fixed_width_two_line')
        valid_table = asc.read(fitspath + filename_dict['bin_valid'], format='fixed_width_two_line')
    ...
    ax7.plot(mass_range, AM_relation, color='g', linestyle='-', alpha=0.5, 
