...
def indiv_bin_info_table(fitspath, line_file, use_revised=False, log=None):
    """
    Purpose:
    ...
    bin_info_npz_file = fitspath + filename_dict['bin_info'].replace('.tbl', '.npz')
    log.info(f"Reading: {bin_info_npz_file}")
    bin_npz = np.load(bin_info_npz_file)

    log.info(f"Reading: {line_file}")
    ...
    valid_tbl = asc.read(valid_file, format='fixed_width_two_line')
        
    # Convert bin info table into a dictionary
    bin_dict = table_to_dict(fitspath + filename_dict['bin_info'], log=log)
    ...
    asc.write(indiv_dict, names=tuple(bin_cols), output=out_ascii,
...
def indiv_em_table(fitspath, line_file, log=None):
    """
    Purpose:
    ...
    npz_info_file = fitspath + filename_dict['bin_info'].replace('.tbl', '.npz')
    log.info(f"Reading: {npz_info_file}")
    bin_npz = np.load(npz_info_file)

    # Get indices valid for binning
    valid_idx = bin_npz['valid_ind_sort']
    
    # Get column names from individual line measurement file
    line_tbl_names = ['_FLUX_MOD', '_FLUX_DATA', '_SNR', '_LAMBDA',
                      '_PEAK', '_Y0', '_SIGMA', '_NOISE']
    ...
    data = {
        indv_names0[0]: line_table['OBJNO'],
        indv_names0[3]: bin_npz['logM'][valid_idx],
    ...
    log.info(f"Writing: {out_ascii}")
    names = tuple([indv_names0[0]] + indv_names0[3:5] + cols)
    asc.write(data, names=names, output=out_ascii,
              format='fixed_width_two_line', overwrite=True)
